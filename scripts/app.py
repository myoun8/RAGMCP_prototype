...
async def lifespan(app: FastAPI):
    ...
    app.state.system_instruction = (
        ...
        "RAG vector database. Each tool's docstring covers its own usage — follow it rather "
        ...
        "\n"
        "REDUCTION WORKFLOW (in order): (1) find_raw_data_paths / list_data_files to get "
        "each file's path+mtime+source; (2) get_file_intent per file if intents are needed "
        "(needs instrument_id, path, mtime, source — take these from step 1; if the user "
        "gives a filename but no path, ask for the path). (3) list_reduction_templates: read "
        "load_nodes for the file→node mapping AND output_nodes for the target_node. Ask which "
        "files to reduce before calling reduce_files. For multi-node templates confirm which "
        "files map to which node/intent (specular/background+/background-/intensity) — never "
        "guess or reuse files across nodes. (4) reduce_files, then (5) plot_reduction to draw "
        "it. Read target_node from output_nodes (the leaf/reduced node) — never guess it; omit "
        "it in plot_reduction only when there is a single leaf node. If reduce_files is not "
        "available, use plot_reduction.\n"
        "\n"
        "PLOTS: render any tool-returned <div class=\"plotly-figure\"> snippet verbatim. Each "
        "rendered plot already carries its own PNG / CSV download buttons — do not build "
        "separate download links for the image or its data.\n"
        "\n"
...
def open_reductus_editor(req: OpenReductusEditorRequest, request: Request):
    ...
    Only works for a client on this same machine: it launches a GUI browser
    window server-side (via Playwright) and injects the template. A remote
    client can't be auto-injected (the app page can't script the cross-origin
    reductus.nist.gov window), so for a non-loopback client we skip Playwright
    and hand back a download_url instead -- the client opens reductus itself and
    the user loads the downloaded template file via reductus's menu -> Load.
    ...
    # Remote client: can't drive its browser from here, so let the client open
    # reductus and load the template file itself (menu -> Load).
