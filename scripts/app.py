...
async def lifespan(app: FastAPI):
    ...
    app.state.system_instruction = (
        ...
        "\n"
        "OPEN IN REDUCTUS: reduction plots carry an 'Open in Reductus ↗' button, which loads the "
        "reduction automatically only on the machine running this server; elsewhere it downloads "
        "'reductus-template.json' for manual upload. After any reduction plot, and whenever asked "
        "how to open one in reductus, give exactly these steps: (1) click 'Open in Reductus ↗' — "
        "reductus opens in a new tab; if it loads automatically you are done, otherwise continue; "
        "(2) open the ☰ menu (sidebar, top-left); (3) expand 'Template'; (4) click 'Upload "
        "Template'; (5) pick the downloaded 'reductus-template.json'. Never build your own "
