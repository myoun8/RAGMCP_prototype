...
- [`mcpServer.py`](scripts/mcpServer.py) — **FastMCP server** exposing twelve tools over stdio: `run_pipeline` (full ingestion pipeline), `gen_chunks` (semantic retrieval from the Chroma vectorstore), nine Reductus tools backed by `reductus.web_gui.api` — `list_instruments`, `get_instrument`, `list_datasources`, `list_data_files` (browse a data source by path), `find_raw_data_paths` (look up an NCNR experiment's raw files by experiment ID/instrument via the NCNR metadata API, with real path/mtime attached), `list_reduction_templates` (find a template's file-input nodes and their intent), `reduce_files` (run selected files through a named reduction template without hand-building the module graph), `get_file_intent` (determine a single raw file's measurement intent from just its header, without a full reduction), and `inspect_raw_file` (open a raw NeXus/HDF5 file and read the free-text descriptions, comments, sample info, and any logged field that live *inside* the file — richer than the harvested NCNR metadata DB — returning curated trial fields, a structure outline, and any explicitly requested NeXus paths) — plus `find_experiment_logsheet` (search the NG7 / NGB30 experiment log-sheet PDF archives on the SANS data share by PI surname, date, or cycle, and return per-match download links served by [`app.py`](scripts/app.py)'s `/download/logsheet` endpoint). Run as `python scripts/mcpServer.py`; consumed by [`agent.py`](scripts/agent.py), [`app.py`](scripts/app.py), and any MCP-compatible client.
...
- **Reductus reduction service** — `list_instruments`, `get_instrument`, `list_datasources`, `list_data_files`, `find_raw_data_paths`, `list_reduction_templates`, `reduce_files`, `get_file_intent`, and `inspect_raw_file` (read descriptions/comments/sample info and arbitrary fields from inside a raw NeXus/HDF5 file), also surfaced as `StructuredTool`s backed by [`mcpServer.py`](scripts/mcpServer.py). The system prompt requires the agent to confirm which files (and, for multi-node templates, which file-to-node/intent assignment) with the user before calling `reduce_files`.
- **SANS experiment log sheets** — `find_experiment_logsheet` searches the NG7 and NGB30 experiment log-sheet PDF archives on the `charlotte.ncnr.nist.gov` SANS data share and returns download links so the agent can hand a matching log-sheet PDF back to the user (files are named by date and PI surname, e.g. `YYYYMMDD_NG7_<PI>.pdf`, so search by PI name or date rather than experiment ID). The web UI serves the PDFs through `app.py`'s `/download/logsheet/{instrument}/{path}` endpoint. All logsheets from 2009 onwards is searchable.
